...
with open("response_1773847564669.json", "r", encoding="utf-8") as f:
    data = json.load(f)
...
for i, line in enumerate(lines):
    ...
    col=random_color()
    for box in line:
        print(f"  Box: {box['quad']} - y_center: {box['y_center']}")
        pts = np.array(box['quad']).astype(int)
        cv2.polylines(path, [pts], True, col, 4)

cv2.imwrite("line_detected.jpg", path)
