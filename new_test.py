...
# ════════════════════════════════════════════════════════════════
# STAGE 3 — WRITE PLAIN-TEXT TRANSCRIPT (ORIGINAL PAGE/LINE ORDER)
# ════════════════════════════════════════════════════════════════

def write_txt_transcript(rows: list[dict], txt_path: str) -> None:
    """
    rows are already in the order they were produced by Stage 1/2, i.e.
    page-by-page (sorted alphabetically by filename in run_pipeline) and,
    within each page, line000, line001, line002, ... in reading order.
    This writes that same order out to a plain .txt file, with a blank
    line + page header whenever the page changes.
    """
    lines_out   = []
    current_pg  = None
    ...
    for row in rows:
        ...
        if page_name != current_pg:
            if current_pg is not None:
                lines_out.append("")   # blank line between pages
            lines_out.append(f"=== {page_name} ===")
            current_pg = page_name
        lines_out.append(row["auto_label"])
...
def run_pipeline():
    ...
    # `rows` is still in original page/line order at this point — this is
    # the order Stage 1 produced (pages sorted alphabetically, lines within
    # a page in line000, line001, ... order). Write the plain-text
    # transcript from this ordered list BEFORE the CSV re-sorts by
    # confidence for review purposes.
    print("\n[ STAGE 3 ] Saving TXT transcript (original page/line order)\n")
    write_txt_transcript(rows, TXT_OUTPUT)
    print(f"  ✓ Transcript saved to {TXT_OUTPUT}")

    print("\n[ STAGE 4 ] Saving CSV (sorted by confidence, for review)\n")
    ...
    print("  6. Read the plain transcript any time in transcript.txt")
    print("=" * 60)
